...
<body left-padding: 10px>
    ...
    <%
        Orders billcheckOut = (Orders)request.getAttribute("billcheckOut");
        Vector<Product> vector =(Vector<Product>)request.getAttribute("itemsConfirmed");
        // Check if the items in cart are not empty
                      if (vector != null && !vector.isEmpty()) {
    %>

    <div>
        <h3>Order ID: <%=billcheckOut.getOrderID()%></h3>
        <h3>Date: <%=billcheckOut.getDateBuy()%></h3>
        <h3>Customer's Name: <%=billcheckOut.getName()%></h3>
        <h3>Customer's Phone: <%=billcheckOut.getPhone()%></h3>
        <h3>Customer's Address: <%=billcheckOut.getAddress()%></h3>           
    </div>
    ...
    <table width=50% border=1 >
        ...
        <tr class="no-border">
            ...
            <td class="no-border"><%= String.format("%.3f", grandTotal) %></td>
        </tr>
